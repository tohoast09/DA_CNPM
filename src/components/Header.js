...
const Header = () => {
    ...
    const handleSearch = (event) => {
        if (activeNav!==0) filterState.changeFilterState('title',event.target.value);
        else filterState.setState(prev=>{return {...prev,title:event.target.value}});
    }
        ...
        <div className="header shrink">
            <div className="container">
                ...
                <div className="header__logo">
                        <img src='https://static.vecteezy.com/system/resources/previews/002/637/558/non_2x/read-book-knowledge-icon-white-background-free-vector.jpg' alt="" />
                ...
                >
                    ...
                    <div className={`header__menu__right ${classes.search}` }>
                        <form onSubmit={(event)=>{
                            ...
                            style={activeNav===0?{border: "1.5px solid black",padding: "10px"}:null}
                        >
                            <input className={classes.search_text} type='text' name="" placeholder="Tim kiem voi BK" onChange={handleSearch} onFocus={onFocus} onBlur={onBlur} style={activeNav===0?{width: "400px", padding: "0px 6px"}:null} ref={searchRef} required />
                            {focused && activeNav!==0 && <DropDownMenuSearch data={res} filterState={filterState} />}
                            <i className={`bx bx-search ${classes.search_btn}`}></i>
                        </form>
                        <div className="header__menu__item header__menu__right__item">

                        <Badge badgeContent={CrtCtx.totalBook} showZero color="primary" overlap="circular">
...
function DropdownItem2(props) {
    return(
    <p className={`${classes.menu_item} searchdropdownitem`} style={{display:"block"}} onMouseDown={props.onClick}>
        {props.children}
    </p>
    )
}
